...
{
public:
	UIPanel(ID3D11Device* device, const Vector2& localPosition, const std::string& dataAsset = "", LoadType loadType = LoadType::kNoLoad, BaseObject* parent = nullptr, float lifeTime = FLT_MAX);
	UIPanel(ID3D11Device* device, const Vector2& size, const Vector2& localPosition, const std::string& dataAsset = "", LoadType loadType = LoadType::kNoLoad, BaseObject* parent = nullptr, float lifeTime = FLT_MAX);
	~UIPanel();
